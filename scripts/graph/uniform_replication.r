...
legend_labels <- c("Basic Replication (batching)", "Basic Replication", "Uniform Replication")
legend_breaks <- c("pvc", "socket_basic", "socket_uniform")
...
df <- read.csv("../../uniform_replication/dynamic/general.csv")
df_keys <- df[df$exp == "keys", ]
df_replicas <- df[df$exp == "replicas", ]
df_fraction <- df[df$exp == "fraction", ]
df_partitions <- df[df$exp == "partitions", ]

partition_plot <- ggplot(df_partitions,
                         aes(x=factor(partitions), y=throughput, group=transport, color=transport)) +
...
keys_plot <- ggplot(df_keys,
                    aes(x=factor(keys), y=throughput, group=transport, color=transport)) +
...
fraction_plot <- ggplot(df_fraction,
                        aes(x=factor(fraction), y=throughput, group=transport, color=transport)) +
...
combined <- grid.arrange(partition_plot + theme(legend.position = "none"),
                         replica_plot + theme(legend.position = "none"),
                         keys_plot + theme(legend.position = "none"),
                         fraction_plot, ncol=1)

ggsave(filename = "./dynamic_uniform_bench.pdf",
       plot = combined,
       device = "pdf",
       width = 5,
       height = 12,
