...
df <- read.csv("../../socket_test/dynamic/general.csv")
df_keys <- df[df$exp == "keys", ]
df_fraction <- df[df$exp == "fraction", ]

keys_plot <- ggplot(df_keys, aes(x=factor(keys), y=throughput, group=transport, color=transport)) +
    geom_point(size=1.5) +
    geom_line() +
...
fraction_plot <- ggplot(df_fraction, aes(x=factor(fraction), y=throughput, group=transport, color=transport)) +
    geom_point(size=1.5) +
    geom_line() +
...
combined <- grid.arrange(keys_plot + theme(legend.position = "none"), fraction_plot, ncol=1)

ggsave(filename = "./dynamic_socket_bench.pdf",
       plot = combined,
       device = "pdf",
       width = 5,
       height = 10,
