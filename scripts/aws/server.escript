...
start_grb(Config) ->
    ...
    EnvVarString = io_lib:format(
        "GRB_QUORUM_OVERRIDE=~b TCP_ID_LEN=~b OP_LOG_READERS=~b VSN_LOG_SIZE=~b RIAK_RING_SIZE=~b SELF_HB_INTERVAL_MS=~b PARTITION_RETRY_MS=~b REPLICATION_INTERVAL_MS=~b PREPARED_BLUE_STALE_MS=~b UNIFORM_REPLICATION_INTERVAL_MS=~b UNIFORM_VISIBILITY_FACTOR=~b BCAST_KNOWN_VC_INTERVAL_MS=~b COMMITTED_BLUE_PRUNE_INTERVAL_MS=~b UNIFORM_CLOCK_INTERVAL_MS=~b RED_HB_INTERVAL_MS=~b RED_DELIVER_INTERVAL_MS=~b RED_PRUNE_INTERVAL=~b RED_LEADER_WAIT_MS=~b RED_COORD_POOL_SIZE=~b VISIBILITY_RATE=~b IP=~s INTER_DC_IP=~s",
        [
            GRB_QUORUM_OVERRIDE,
            TCP_ID_LEN,
            OP_LOG_READERS,
